	...
	for (int i = 0; i < oReader.triangles.size(); ++i) {
		scene.triangles.push_back(
				Body<Triangle>(oReader.triangles[i],
						new Material(Vector3d(0.5, 0.9, 0.5),
								Vector3d(0, 0, 0))));
//								new Chrome(
//										Vector3d(0.8,
//												0.8,
//												0.8))));
	...
	cout << "Indexing triangles" << endl;
	scene.indexTriangles();
	cout << "Indexing triangles ended." << endl;
...
//														0.9))));
	...
	scene.planes.push_back(
			Body<Plane>(Plane(Vector3d(0.0, 0.0, 1), -1),
					new Material(Vector3d(0.9, 0.9, 0.9), Vector3d(0, 0, 0))));
...
//		//resize(img / (i + 1), out, Size(), 2, 2);
		out = img / (i + 1);
//		imshow("out", out);
		imwrite("render.jpg", (img / (i + 1)) * 255);
//		char c = waitKey(1);
//		if (c == 27) {
//			break;
//		}
